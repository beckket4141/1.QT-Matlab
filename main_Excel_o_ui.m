...
for i = 1:length(selected_files)
    ...
    if exist(full_filename, 'file')
        ...
        % 绘制相图并保存（需要传入保存路径）
        output_path = base_path; % 临时使用输入路径，后续UI化时会改为用户选择的输出路径
        phase_final = mapsave(rho_final, dimension, output_path, ['file_' num2str(base_number)]);
        ...
        % 保存密度矩阵和结果
        save_density_matrix_results(rho_final, first_chi2, final_chi2, purity1, purity2, output_path, base_number);
