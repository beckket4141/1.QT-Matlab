function quantum_tomography_ui()
    % 量子态层析数据处理UI界面
    
    % 创建主窗口
    fig = uifigure('Position', [100 100 600 700]);
    fig.Name = '量子态层析数据处理工具';
    fig.Resize = 'off';
    
    % 创建UI组件
    createComponents(fig);
end

function createComponents(fig)
    % 创建所有UI组件
    
    % 标题
    title_label = uilabel(fig);
    title_label.Position = [200 650 200 30];
    ...
    input_panel.Position = [20 450 560 180];
    ...
    data_path_edit.Position = [100 130 350 22];
    data_path_edit.Tag = 'data_path';
    
    data_path_btn = uibutton(input_panel, 'push');
    data_path_btn.Position = [460 130 80 22];
    ...
    output_panel.Position = [20 350 560 80];
...
function saveDefaultConfig(fig)
    ...
    try
        ...
        save('quantum_tomography_config.mat', 'config');
        
        log_area = findobj(fig, 'Tag', 'log_textarea');
        current_log = log_area.Value;
        current_log{end+1} = ['[' datestr(now, 'HH:MM:SS') '] 配置已保存为默认值'];
        log_area.Value = current_log;
        
        uialert(fig, '配置已成功保存为默认值', '保存成功', 'Icon', 'success');
    catch ME
...
function loadDefaultConfig(fig)
    % 加载默认配置
    if exist('quantum_tomography_config.mat', 'file')
        try
            load('quantum_tomography_config.mat', 'config');
...
function startProcessing(fig)
    ...
    try
        ...
        % 调用处理函数
        processQuantumTomography(params, fig);
...
function processQuantumTomography(params, fig)
    % 调用量子层析处理程序
    ...
    for i = 1:total_files
        ...
        % 调用处理函数
        processingleFile(file_name, params, @updateLog);
...
function processingleFile(file_name, params, updateLog)
    % 处理单个文件 - 这里调用修改后的主程序逻辑
